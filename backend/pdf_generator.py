...
""",
    ...
    def generate_api_endpoints_guide(self):
        """Generate API endpoints reference guide"""
        
        return {
            "title": "API Endpoints Reference - Complete Technical Documentation",
            "content": """
# 🔗 API ENDPOINTS REFERENCE
...
```bash
curl -X POST "https://digital-launch-pro.preview.emergentagent.com/api/products" \
...
```bash
curl -X POST "https://digital-launch-pro.preview.emergentagent.com/api/automation/generate-content" \
  -H "Content-Type: application/json" \
  -d '{
    "platform": "instagram",
    "theme": "productivity"
...
```bash
curl -X POST "https://digital-launch-pro.preview.emergentagent.com/api/emails/generate-content?email_type=welcome" \
  -H "Content-Type: application/json" \
  -d '{
